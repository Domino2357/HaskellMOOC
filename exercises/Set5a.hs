...
totalPrice (MkShoppingEntry _ p a) = p*fromIntegral a
...
buyOneMore (MkShoppingEntry n p a) = MkShoppingEntry n p (a+1)
...
study student = case student of
  Freshman -> NthYear 1
  NthYear i -> if i < 7  then NthYear (i+1) else Graduated
  Graduated -> Graduated
...
rgb col = case col of
  ...
  Green -> [0,1,0]
  Blue -> [0,0,1]
  Mix a b -> mix (rgb a) (rgb b)
  Invert c -> invert (rgb c)
...
fromNat n = case n of
  Zero -> 0
  PlusOne nMinusOne-> 1 + fromNat nMinusOne
...
toBin n
  | n==1 = I End
  | n==0  = O End
  | n `mod` 2 == 0 = O (toBin (floor (fromIntegral n/2)))
  | n `mod` 2 == 1 = I (toBin (floor (fromIntegral n/2)))
  | otherwise = O End
