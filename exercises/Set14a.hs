...
greetText text = if (T.length text) > 15
                    then  ((T.pack "Hello, ") <> T.take 15 text) <> (T.pack "...!")
                else ((T.pack "Hello, ") <> text <> (T.pack "!"))
...
shout :: T.Text -> T.Text
shout text = T.intercalate (T.pack " ") (capitalizeSecond True (T.words text))

capitalizeSecond :: Bool -> [T.Text] -> [T.Text]
capitalizeSecond cond [] = []
capitalizeSecond cond (x:xs) = if cond 
                                    then T.toUpper(x):(capitalizeSecond False xs)
                                else x:(capitalizeSecond True xs) 
------------------------------------------------------------------------------
...
longestRepeat text = helpLongestRepeat 0 0 Nothing (T.unpack text)

helpLongestRepeat ::  Int -> Int -> Maybe Char -> String -> Int
helpLongestRepeat curMax cur c [] = if cur > curMax then cur else curMax
helpLongestRepeat curMax cur c (x:xs) = 
    case c of
        Nothing -> 
            helpLongestRepeat curMax 1 (Just x) xs
        Just ch ->
            if ch == x then
                helpLongestRepeat curMax (cur + 1) (Just ch) xs
            else
                if cur > curMax then
                    helpLongestRepeat cur 1 (Just x) xs
                else
                    helpLongestRepeat curMax 1 (Just x) xs 
...
byteRange b = if b == B.empty then (B.head (B.pack [0])) else (B.head(B.reverse(bSorted))-B.head(bSorted))
    where bSorted = B.sort b
...
--   xor 1 (xor 2 4) ==> 7(B.head (B.pack [0]))
...
xorChecksum b = B.foldl (Data.Bits.xor) (B.head (B.pack [0])) b
...
countUtf8Chars b = case decoded of
    Left u -> Nothing
    Right t -> Just (T.length t)
    where decoded = Data.Text.Encoding.decodeUtf8' b
