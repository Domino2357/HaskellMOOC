...
combine f (Picture p1) (Picture p2) = Picture p 
  where p (Coord x y) = f (p1 (Coord x y)) (p2 (Coord x y)) 
------------------------------------------------------------------------------
...
union (Shape f) (Shape g) = Shape f_union_g
    where f_union_g (Coord x y) = f (Coord x y) || g (Coord x y)

cut :: Shape -> Shape -> Shape
cut (Shape f) (Shape g) = Shape f_cut_g
    where f_cut_g (Coord x y) = not (f (Coord x y) && g (Coord x y)) && f (Coord x y)
------------------------------------------------------------------------------
...
paintSolid color (Shape f) (Picture g) = Picture p
    where p c = if f c then color else g c
------------------------------------------------------------------------------
...
paint (Picture b) (Shape f) (Picture g) = Picture p
    where p c = if f c then b c else g c
------------------------------------------------------------------------------
...
instance Transform Flip where
  ...
  apply flip (Picture g) = case flip of
    FlipX -> Picture f where f (Coord x y) = g (Coord (-x) y) 
    FlipY -> Picture f where f (Coord x y) = g (Coord x (-y))
    FlipXY -> flipXY (Picture g)
------------------------------------------------------------------------------
...
instance Transform Blur where
  ...
  apply blur (Picture f) = Picture g
    where g (Coord x y) = colorDivision (addColors (f (Coord x y)) (addColors (f (Coord (x-1) y)) (addColors (f (Coord (x+1) y)) (addColors (f (Coord x (y-1))) (f (Coord x (y+1))))))) 5


addColors :: Color -> Color -> Color
addColors (Color  r1 b1 g1) (Color r2 b2 g2) = Color (r1 +r2) (b1 + b2) (g1 + g2)

colorDivision :: Color -> Int -> Color
colorDivision (Color r b g) i = Color (r `div` i) (b `div` i) (g `div` i)
------------------------------------------------------------------------------
