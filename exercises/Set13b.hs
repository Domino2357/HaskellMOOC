...
mapM2 op _ [] = return []
mapM2 op [] _ = return []
mapM2 op (x:xs) (y:ys) = op x y >>= (\y -> fmap (y :) (mapM2 op xs ys))
...
visit maze place = do
                      currentState <- get
                      if place `elem` currentState then return ()
                      else 
                        do
                          modify (\x -> place:x)
                          forM_ (findNeighbors maze place) (\n -> visit maze n)

findNeighbors :: [(String, [String])] -> String -> [String]
findNeighbors [] place = []
findNeighbors ((a,b):xs) place = if a == place then b else findNeighbors xs place
...
path maze place1 place2 = if place2 `elem` visitedNodes then True else False
                          where
                            visitedNodes = snd (runState (visit maze place1) [])
...
findSum2 _ [] = []
findSum2 [] _ = []
findSum2 ks (n:ns) = (findSum ks n) ++ (findSum2 ks ns)

findSum :: [Int] -> Int -> [(Int,Int, Int)]
findSum xs k = do a <- xs
                  b <- xs
                  if (a+b==k) then [(a,b,k)] else []
...
f1 k acc x = if sum <= k then Just sum
              else Nothing
              where sum = acc + x
...
f2 acc x = do currentState <- get
              modify (\a -> (x:a)) 
              if x `elem` currentState
                then
                  return acc
              else 
                return (x + acc)
...
--modifyFunction :: (a -> b) ->  (Int -> (a, Int, String)) -> (Int -> (b, Int, String))
helperFunctionFunctor:: Int -> (a -> b) -> (Int -> (a, Int, [String])) -> ((b, Int, [String]))
helperFunctionFunctor x f g = (f a, b, c)
                      where (a, b, c) = g x  

instance Functor SL where
  -- implement fmap
  fmap f (SL g) = SL (\x -> (helperFunctionFunctor x f g))
...
helper1 :: (a, Int, [String]) -> (a -> SL b) -> (b, Int, [String])
helper1 (c, d, e) f = (l, m, e++n)
  where
    (l, m, n) = helper3 d (f c)

helper2:: Int -> (Int-> (a, Int, [String])) -> (a, Int, [String])
helper2 x g = g x

helper3:: Int -> SL b -> (b, Int, [String])
helper3 x (SL g) = g x

instance Monad SL where
  -- implement return and >>=
  return a = SL (\x -> (a, x, []))
  (>>=) (SL g) f = SL(\x -> (helper1 ((helper2 x g)) f))
