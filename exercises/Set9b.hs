...
-- I don't think the challenge is possible to do with a strict definition of the
-- algorithmic complexity. Traversing the coordinates is
-- already in O(n^2). Worst case scenario, we'd need to travers the list at every coordinate
-- completely, which gives O(n^3). We could of course sort the list but that in itself is 
-- already in O(n*log(n)) for one dimensional coordinates, we have 2. 
-- Also, we could prewrite the string and "modify" it at each queen
-- coordinate, but then we'd need to travers the string in the worst case n times times n^2
-- which gives n^3 again.
-- Maybe it's possible to do one of these steps very efficiently in Haskell, rendering
-- the algorithm to be in O(n^2) but since I don't, I'll go with 'elem# instead. 
...
fixFirst n (x:xs) = 
    case moved of
        Nothing -> Nothing
        Just saveSpot -> Just (saveSpot:xs)
    where moved = moveOutOfDanger x n xs

moveOutOfDanger:: Coord -> Size -> Stack -> Maybe Coord
moveOutOfDanger (i, j) n queens =
    if danger (i, j) queens || j > n
        then if j >= n then Nothing
            else moveOutOfDanger (nextCol (i,j)) n queens
    else Just (i,j)
--------------------------------------------------------------------------------
...
step n s =
    case quickFixed of
        Nothing -> (backtrack s)
        Just validStack -> continue validStack
    where quickFixed = fixFirst n s
...
finish n s = 
    let nextStep = (step n s) in
    if length nextStep == n+1 then drop 1 nextStep 
