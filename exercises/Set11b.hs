...
appendAll ref [] = do
    return ()
appendAll ref (s:strings) = do
    cur <- readIORef ref
    let new = cur ++ s
    writeIORef ref new
    res <- appendAll ref strings
    return res
...
interact' f state = do
    read <- getLine
    let input = (read, state)
    let (check, str, stateNext) = f input
    putStrLn str 
    if check then do
        interact' f stateNext
    else
        return stateNext