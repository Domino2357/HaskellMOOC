...
import Mooc.Todo
import Database.SQLite.Simple (nextRow)
...
workload nExercises hoursPerExercise =
  if totalWorkingTime > 100 then "Holy moly!"
  else if totalWorkingTime < 10 then "Piece of cake!"
  else "Ok."
  where totalWorkingTime = nExercises*hoursPerExercise
...
countValid [] = 0
countValid (x:xs) =
  if length x >= 5 && ((x !! 2 == x !! 4) || (x !! 3 == x !! 5)) then 1 + next
  else next
  where next = countValid xs
...
sumSuccess (x:xs) = case x of
  Right dataPoint -> case sumSuccess xs of
    Right next -> Right (dataPoint + next)
    Left errorMessage -> Right dataPoint
  Left errorMessage -> sumSuccess xs
...
isOpen :: Lock -> Bool
isOpen (Lock isOpen code) = isOpen

-- open tries to open the lock with the given code. If the code is
-- wrong, nothing happens.
open :: String -> Lock -> Lock
open str (Lock isOpen code) = if str == code then Lock True code
else Lock isOpen code

-- lock closes a lock. If the lock is already closed, nothing happens.
lock :: Lock -> Lock
lock (Lock isOpen code) = Lock False code

-- changeCode changes the code of an open lock. If the lock is closed,
-- nothing happens.
changeCode :: String -> Lock -> Lock
changeCode newCode (Lock isOpen code) = if isOpen then Lock isOpen newCode else Lock isOpen code
...
instance Eq Text where
  Text [] == Text [] = True
  Text str1 == Text str2 = stripWhiteSpaces str1 == stripWhiteSpaces str2

stripWhiteSpaces:: String -> String
stripWhiteSpaces [] = []
stripWhiteSpaces (x:xs) = if isSpace x then stripWhiteSpaces xs else x:stripWhiteSpaces xs
...
compose xs [] = []
compose [] xs = []
compose (x:xs) ys = case valueAtX of
  Nothing -> compose xs ys
  Just val -> (fst x, val):compose xs ys
  where valueAtX = lookup (snd x) ys
...
permute :: (Ord a, Eq a) => Permutation -> [a] -> [a]
permute [] [] = []
permute xs [] = []
permute [] xs = xs
permute p ys = snd (unzip (sortBy (\(a, _) (b, _)-> compare  a b) (zip p ys)))