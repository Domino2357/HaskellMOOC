...
balance db account = do
    all <- query db balanceQuery [account] :: IO [[Int]]
    return $ sum $ map sum all
...
parseCommand (command:account:maybeAmount) = if maybeAmount == [] then Just (Balance account)
  else
    case parseInt (head maybeAmount) of
